...
network_analyzer() {
    ...
    echo -e "  ${CYAN}[8]${RESET} IP Geolocation"
...
ip_geolocation() {
    ...
    echo ""
    echo -ne "  ${WHITE}Enter IP or Domain (leave empty for self): ${RESET}"
    read -r target

    echo ""
    echo -e "  ${CYAN}Fetching data...${RESET}"
    
    local geo_data
    geo_data=$(curl -s "http://ip-api.com/json/${target}?fields=status,message,country,countryCode,regionName,city,zip,lat,lon,timezone,isp,org,as,query" 2>/dev/null)

    if [[ $(echo "$geo_data" | grep -o '"status":"success"') ]]; then
        print_section "Results for: $(echo "$geo_data" | grep -oP '"query":"\K[^"]*')"
        print_kv "Location" "$(echo "$geo_data" | grep -oP '"city":"\K[^"]*'), $(echo "$geo_data" | grep -oP '"country":"\K[^"]*') ($(echo "$geo_data" | grep -oP '"countryCode":"\K[^"]*'))"
        print_kv "Region" "$(echo "$geo_data" | grep -oP '"regionName":"\K[^"]*') ($(echo "$geo_data" | grep -oP '"zip":"\K[^"]*'))"
        print_kv "Coordinates" "$(echo "$geo_data" | grep -oP '"lat":\K[^,]*'), $(echo "$geo_data" | grep -oP '"lon":\K[^}]*')"
        print_kv "ISP" "$(echo "$geo_data" | grep -oP '"isp":"\K[^"]*')"
        print_kv "Organization" "$(echo "$geo_data" | grep -oP '"org":"\K[^"]*')"
        print_kv "ASN" "$(echo "$geo_data" | grep -oP '"as":"\K[^"]*')"
        print_kv "Timezone" "$(echo "$geo_data" | grep -oP '"timezone":"\K[^"]*')"
        
        local lat=$(echo "$geo_data" | grep -oP '"lat":\K[^,]*')
        local lon=$(echo "$geo_data" | grep -oP '"lon":\K[^}]*')
        echo ""
        echo -e "  ${DIM}Maps Link: https://www.google.com/maps?q=${lat},${lon}${RESET}"
        print_section_end
    else
        echo -e "  ${RED}Error: Failed to retrieve geolocation data.${RESET}"
    fi
...
toolkit() {
    ...
    echo -e "  ${CYAN}[13]${RESET} Termux Package Manager"
    echo -e "  ${CYAN}[13]${RESET} Android App Explorer"
    echo -e "  ${CYAN}[14]${RESET} File Encrypt/Decrypt"
    echo -e "  ${CYAN}[15]${RESET} Website Availability"
    echo -e "  ${CYAN}[16]${RESET} Unit Converter"
    echo -e "  ${CYAN}[17]${RESET} Simple Calculator"
    echo -e "  ${CYAN}[18]${RESET} Countdown Timer"
    echo -e "  ${CYAN}[19]${RESET} Random String Generator"
    echo -e "  ${CYAN}[20]${RESET} URL Shortener"
    echo -e "  ${CYAN}[21]${RESET} Weather Forecast"
    echo -e "  ${CYAN}[22]${RESET} Text File Diff"
    echo -e "  ${CYAN}[23]${RESET} Local Port Forwarding"
    echo -e "  ${CYAN}[0]${RESET}  Back"
    echo ""
    echo -ne "  ${WHITE}Choice: ${RESET}"
    read -r tool_choice
    ...
    case $tool_choice in
        ...
        12) system_cleaner ;;
        13) termux_package_manager ;;
        13) app_explorer ;;
        14) file_crypt ;;
        15) website_availability ;;
        16) unit_converter ;;
        17) simple_calculator ;;
        18) countdown_timer ;;
        19) random_string_generator ;;
        20) url_shortener ;;
        21) weather_forecast ;;
        22) text_diff_tool ;;
        23) local_port_forwarding ;;
        0) return ;;
        *) echo -e "  ${RED}Invalid choice${RESET}"; sleep 1 ;;
    esac
}
...
app_explorer() {
    clear
    print_header "📱 ANDROID APP EXPLORER"
    echo ""
    
    echo -ne "  ${WHITE}Search package (empty for all): ${RESET}"
    read -r search_term

    echo ""
    echo -e "  ${CYAN}Querying package manager...${RESET}"
    echo ""
    
    local apps
    if [[ -z "$search_term" ]]; then
        apps=$(pm list packages -3 2>/dev/null | cut -d: -f2 | sort | head -20)
        echo -e "  ${DIM}Showing first 20 user-installed apps:${RESET}\n"
    else
        apps=$(pm list packages 2>/dev/null | grep "$search_term" | cut -d: -f2 | sort | head -20)
        echo -e "  ${DIM}Search results for '$search_term':${RESET}\n"
    fi
    ...
    if [[ -z "$apps" ]]; then
        ...
        while IFS= read -r pkg; do
            local version
            version=$(dumpsys package "$pkg" 2>/dev/null | grep -m1 "versionName=" | cut -d= -f2)
            printf "  ${WHITE}● %-40s${RESET} ${CYAN}%s${RESET}\n" "${pkg:0:40}" "${version:-unknown}"
        done <<< "$apps"
    fi

    echo ""
    echo -e "  ${CYAN}[1]${RESET} View details of a package"
    echo -e "  ${CYAN}[0]${RESET} Back"
    echo ""
    echo -ne "  ${WHITE}Choice: ${RESET}"
    read -r app_choice
    ...
    if [[ "$app_choice" == "1" ]]; then
        echo -ne "  ${WHITE}Full package name: ${RESET}"
        read -r target_pkg
        [[ -n "$target_pkg" ]] && dumpsys package "$target_pkg" | head -n 50 | less
    fi
}
...
file_crypt() {
    ...
    if ! command -v openssl &>/dev/null; then
        echo -e "  ${YELLOW}Requires openssl. Install with: pkg install openssl${RESET}"
        echo ""
        echo -ne "  ${DIM}Press Enter to continue...${RESET}"
        read -r
        return
    ...
    echo -e "  ${CYAN}[1]${RESET} Encrypt file"
    echo -e "  ${CYAN}[2]${RESET} Decrypt file"
    echo -e "  ${CYAN}[0]${RESET} Back"
    echo ""
    echo -ne "  ${WHITE}Choice: ${RESET}"
    read -r crypt_choice
    ...
    case $crypt_choice in
        1)
            echo -ne "  ${WHITE}File to encrypt: ${RESET}"
            read -r input_file
            ...
            echo -e "  ${CYAN}Encrypting... (you will be prompted for a password)${RESET}"
            openssl enc -aes-256-cbc -salt -in "$input_file" -out "$output_file"
            if [[ $? -eq 0 ]]; then
                echo -e "  ${GREEN}✓ File encrypted to $output_file${RESET}"
            else
                echo -e "  ${RED}✗ Encryption failed${RESET}"
            fi
            ;;
        2)
            echo -ne "  ${WHITE}File to decrypt: ${RESET}"
            read -r input_file
            if [[ ! -f "$input_file" ]]; then
                echo -e "  ${RED}File not found${RESET}"
                sleep 1; return
            ...
            local output_file="${input_file%.enc}"
            [[ "$output_file" == "$input_file" ]] && output_file="${input_file}.dec"
            echo -e "  ${CYAN}Decrypting... (you will be prompted for a password)${RESET}"
            openssl enc -aes-256-cbc -d -in "$input_file" -out "$output_file"
            if [[ $? -eq 0 ]]; then
                echo -e "  ${GREEN}✓ File decrypted to $output_file${RESET}"
            else
                echo -e "  ${RED}✗ Decryption failed (incorrect password or corrupted file?)${RESET}"
            fi
            ;;
    esac
    echo ""
...
website_availability() {
    ...
    echo -ne "  ${WHITE}Enter URL (e.g., google.com): ${RESET}"
    read -r url
    ...
    echo ""
    echo -e "  ${CYAN}Checking availability of ${WHITE}$url${CYAN}...${RESET}"
    echo ""

    local http_code
    http_code=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 "$url")
    local dns_lookup_time=$(curl -s -o /dev/null -w "%{time_namelookup}" --max-time 10 "$url")
    local connect_time=$(curl -s -o /dev/null -w "%{time_connect}" --max-time 10 "$url")
    local total_time=$(curl -s -o /dev/null -w "%{time_total}" --max-time 10 "$url")

    print_section "Results for: $url"
    print_kv "HTTP Status" "$http_code"
    print_kv "DNS Lookup" "${dns_lookup_time}s"
    print_kv "Connect Time" "${connect_time}s"
    print_kv "Total Time" "${total_time}s"

    if [[ "$http_code" -ge 200 && "$http_code" -lt 400 ]]; then
        echo -e "  ${GREEN}✓ Website is UP and accessible.${RESET}"
    else
        echo -e "  ${RED}✗ Website might be DOWN or inaccessible (HTTP Status: $http_code).${RESET}"
    fi
    print_section_end
...
unit_converter() {
    ...
    if ! command -v bc &>/dev/null; then
        echo -e "  ${YELLOW}Requires bc. Install with: pkg install bc${RESET}"
        echo ""
        echo -ne "  ${DIM}Press Enter to continue...${RESET}"
        read -r
        return
    ...
    echo -e "  ${CYAN}Convert:${RESET}"
    echo -e "  ${CYAN}[1]${RESET} Celsius to Fahrenheit"
    echo -e "  ${CYAN}[2]${RESET} Fahrenheit to Celsius"
    echo -e "  ${CYAN}[3]${RESET} Kilometers to Miles"
    echo -e "  ${CYAN}[4]${RESET} Miles to Kilometers"
    echo -e "  ${CYAN}[0]${RESET} Back"
    echo ""
    echo -ne "  ${WHITE}Choice: ${RESET}"
    read -r conv_choice

    echo -ne "  ${WHITE}Enter value: ${RESET}"
    read -r value

    local result
    case $conv_choice in
        1) result=$(echo "scale=2; ($value * 9 / 5) + 32" | bc); print_kv "Result" "${result}°F" ;;
        2) result=$(echo "scale=2; ($value - 32) * 5 / 9" | bc); print_kv "Result" "${result}°C" ;;
        3) result=$(echo "scale=2; $value / 1.60934" | bc); print_kv "Result" "${result} miles" ;;
        4) result=$(echo "scale=2; $value * 1.60934" | bc); print_kv "Result" "${result} km" ;;
        0) return ;;
        *) echo -e "  ${RED}Invalid choice${RESET}" ;;
    esac
...
simple_calculator() {
    ...
    echo -ne "  ${WHITE}Enter expression (e.g., 5 + 3 * 2 / 4): ${RESET}"
    read -r expression

    if [[ -z "$expression" ]]; then
        echo -e "  ${RED}No expression entered${RESET}"
        sleep 1; return
    fi
    ...
    if [[ -n "$result" ]]; then
        ...
        echo -e "  ${RED}Invalid expression or calculation error.${RESET}"
    fi
...
countdown_timer() {
    ...
    echo -ne "  ${WHITE}Enter duration in seconds: ${RESET}"
    read -r duration

    if ! [[ "$duration" =~ ^[0-9]+$ ]] || (( duration <= 0 )); then
        echo -e "  ${RED}Invalid duration. Please enter a positive integer.${RESET}"
        sleep 1; return
    fi

    echo ""
    echo -e "  ${CYAN}Starting countdown for ${duration} seconds...${RESET}"
    echo ""
    ...
    while true; do
        ...
        if (( remaining <= 0 )); then
            printf "\r  ${GREEN}🎉 Time's up! ${RESET}\n"
            break
        fi
        ...
        printf "\r  ${WHITE}Remaining: %02d:%02d${RESET}" "$minutes" "$seconds"
        sleep 1
    done
...
random_string_generator() {
    ...
    echo -ne "  ${WHITE}String length [16]: ${RESET}"
    read -r str_length
    str_length=${str_length:-16}

    echo -ne "  ${WHITE}Number of strings [5]: ${RESET}"
    read -r str_count
    str_count=${str_count:-5}
    ...
    echo ""
    print_section "Generated Strings"
    echo ""
    ...
    for ((i=1; i<=str_count; i++)); do
        local random_string
        random_string=$(tr -dc "$charset" < /dev/urandom 2>/dev/null | head -c "$str_length")
...
url_shortener() {
    ...
    echo -ne "  ${WHITE}Enter long URL: ${RESET}"
    read -r long_url
    ...
    echo ""
    echo -e "  ${CYAN}Shortening URL using TinyURL...${RESET}"

    local short_url
    short_url=$(curl -s "http://tinyurl.com/api-create.php?url=$long_url")

    if [[ "$short_url" =~ ^http ]]; then
        print_kv "Original URL" "$long_url"
        print_kv "Shortened URL" "${GREEN}${short_url}${RESET}"
    else
        echo -e "  ${RED}✗ Failed to shorten URL. Error: $short_url${RESET}"
    fi
...
weather_forecast() {
    clear
    print_header "☁️ WEATHER FORECAST"
    echo ""

    echo -ne "  ${WHITE}Enter city or ZIP code (empty for auto-detect): ${RESET}"
    read -r location

    echo ""
    echo -e "  ${CYAN}Fetching weather data...${RESET}"
    echo ""

    local weather_data
    if [[ -z "$location" ]]; then
        weather_data=$(curl -s "wttr.in?0" 2>/dev/null)
    else
        weather_data=$(curl -s "wttr.in/$location?0" 2>/dev/null)
    fi

    if [[ -n "$weather_data" ]]; then
        echo "$weather_data" | head -n 10 # Display first 10 lines for brevity
    else
        echo -e "  ${RED}✗ Failed to retrieve weather data. Check location or network.${RESET}"
    fi
...
text_diff_tool() {
    clear
    print_header "📝 TEXT FILE DIFFER"
    echo ""

    echo -ne "  ${WHITE}Enter path to first file: ${RESET}"
    read -r file1
    echo -ne "  ${WHITE}Enter path to second file: ${RESET}"
    read -r file2

    if [[ ! -f "$file1" ]]; then
        echo -e "  ${RED}First file not found: $file1${RESET}"
        sleep 1; return
    fi
    if [[ ! -f "$file2" ]]; then
        echo -e "  ${RED}Second file not found: $file2${RESET}"
        sleep 1; return
    fi

    echo ""
    echo -e "  ${CYAN}Comparing files...${RESET}"
    echo ""
...
local_port_forwarding() {
    clear
    print_header "↔️ LOCAL PORT FORWARDING (SSH)"
    echo ""

    if ! command -v ssh &>/dev/null; then
        echo -e "  ${YELLOW}Requires OpenSSH. Install with: pkg install openssh${RESET}"
        echo ""
        echo -ne "  ${DIM}Press Enter to continue...${RESET}"
        read -r
        return
    ...
    echo -e "  ${CYAN}This tool helps set up an SSH local port forward.${RESET}"
    echo -e "  ${DIM}Example: Forward local port 8080 to remote_host:80 via ssh_user@ssh_server${RESET}"
    echo ""

    echo -ne "  ${WHITE}Local Port (e.g., 8080): ${RESET}"
    read -r local_port
    echo -ne "  ${WHITE}Remote Host (target, e.g., example.com): ${RESET}"
    read -r remote_host
    echo -ne "  ${WHITE}Remote Port (target, e.g., 80): ${RESET}"
    read -r remote_port
    echo -ne "  ${WHITE}SSH Server (e.g., user@ssh.example.com): ${RESET}"
    read -r ssh_server

    if [[ -z "$local_port" || -z "$remote_host" || -z "$remote_port" || -z "$ssh_server" ]]; then
    ...
    echo -e "  ${WHITE}ssh -L $local_port:$remote_host:$remote_port $ssh_server${RESET}"
    echo ""
    echo -e "  ${YELLOW}This will open an SSH connection and keep it open.${RESET}"
    echo -e "  ${YELLOW}You can then access ${remote_host}:${remote_port} via localhost:${local_port}.${RESET}"
    echo ""

    if confirm "Execute this SSH command?"; then
        echo -e "  ${CYAN}Starting SSH tunnel... (Press Ctrl+C to stop)${RESET}"
